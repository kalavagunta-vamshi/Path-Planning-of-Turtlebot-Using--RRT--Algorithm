...
class Plotting:
    ...
    def plot_path(self, path):
        if path:
            plt.plot([x[0] for x in path], [x[1] for x in path], '-r', linewidth=2, alpha=0.8)
            plt.pause(0.01)
        plt.show()
...
class RRTConnect:
    ...
    def planning(self):
        ...
        for i in range(self.maximum_iterations):
            iter_count += 1
            q_rand = self.generate_random_node(self.s_goal, self.goal_sample_rate)
            q_near = self.nearest_neighbor(self.V1, q_rand)
            q_new = self.new_state(q_near, q_rand)

            if q_new and not self.helpers.is_collision(q_near, q_new):
                self.V1.append(q_new)
                node_near_prim = self.nearest_neighbor(self.V2, q_new)
                node_new_prim = self.new_state(node_near_prim, q_new)

                if node_new_prim and not self.helpers.is_collision(node_new_prim, node_near_prim):
                    self.V2.append(node_new_prim)

                    while True:
                        node_new_prim2 = self.new_state(node_new_prim, q_new)
                        if node_new_prim2 and not self.helpers.is_collision(node_new_prim2, node_new_prim):
                            self.V2.append(node_new_prim2)
                            node_new_prim = self.change_node(node_new_prim, node_new_prim2)
                        else:
                            break

                        if self.is_node_same(node_new_prim, q_new):
                            break

                total_nodes_visited = len(self.V1) + len(self.V2)  # count nodes visited

                if self.is_node_same(node_new_prim, q_new):
                    return self.extract_path(q_new, node_new_prim), iter_count, total_nodes_visited
        ...
        return None, iter_count, total_nodes_visited
...
rrt_conn = RRTConnect(x_start, x_goal, 0.5, 0.05, 10000)
path, iterations, nodes = rrt_conn.planning()
end_time = time.time()

flag = False
if(path[0][0]>(x_start[0]+1)):
    flag = True
