...
class RRT:
    ...
    def generate_random_node(self, goal_sample_rate, bias=0.8):
        delta = self.helpers.delta

        if np.random.random() < bias:
            return self.s_goal

        node_near = self.nearest_neighbor(self.vertex, self.s_goal)
        dist, _ = self.get_distance_and_angle(node_near, self.s_goal)

        if np.random.random() > goal_sample_rate or dist < self.step_len:
...
end_time = time.time()
path_reveresed = path[::-1]
print(path_reveresed)
...
if path:
    rrt.plotting.animation(rrt.vertex, path, "Goal_biased_RRT", True)
